...
class MyVGG(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc01 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len1, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc02 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len2, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc03 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc12 = nn.Sequential(
            nn.Linear(self.mul_len1+self.mul_len2, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc13 = nn.Sequential(
            nn.Linear(self.mul_len3+self.mul_len1, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc23 = nn.Sequential(
            nn.Linear(self.mul_len2+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc012 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len1+self.mul_len2, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc013 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len1+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        )
        self.fc023 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len2+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        ...
        self.fc123 = nn.Sequential(
            nn.Linear(self.mul_len2+self.mul_len1+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
        )
        self.fc0123 = nn.Sequential(
            nn.Linear(self.num_genes+self.mul_len1+self.mul_len2+self.mul_len3, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1),
    ...
    def forward(self, x, z1, z2, z3):
        if self.step == "s0":
            ...
            data_1 = gene_outputs
            x = self.fc0_sigle(data_1)
            return x
        if self.step == "s1":
            data_1 = z1
            x = self.fc1_sigle(data_1)
            return x
        if self.step == "s2":
            data_1 = z2
            x = self.fc2_sigle(data_1)
            return x
        if self.step == "s3":
            data_1 = z3
            x = self.fc3_sigle(data_1)
            return x
        if self.step == "s01":
            # 选择对应的特征并通过全连接层处理
            gene_outputs = []
            ...
            gene_outputs = torch.cat(gene_outputs, dim=1)
            data_1 = z1
            data_1 = torch.cat((data_1, gene_outputs), dim=1)
            x = self.fc01(data_1)
            return x
        if self.step == "s02":
            # 选择对应的特征并通过全连接层处理

...
class Mydataset_xyz123(Dataset):
    """设置生成器，批次预处理再进入模型"""
    def __init__(self, x, y, z1, z2, z3):
        self.x = x
        self.y = y
        self.z1 = z1
        self.z2 = z2
    ...
    def __getitem__(self, item):
        ...
        # x = np.expand_dims(np.array(x), axis=0)
        x = torch.from_numpy(x).type(torch.float)
        z1 = torch.from_numpy(z1).type(torch.float)
        z2 = torch.from_numpy(z2).type(torch.float)
        z3 = torch.from_numpy(z3).type(torch.float)
        y = y.astype(np.float32)
        y = torch.from_numpy(y).type(torch.float)
        return x, y, z1, z2, z3

...
try:
    ...
    ture_test, pre_test = [], []
    with torch.no_grad():
        for tes_j, (tes_x, tes_y, tes_z1, tes_z2, tes_z3) in enumerate(test_dl):
            tes_x, tes_y = tes_x.to(device), tes_y.to(device)
            tes_z1, tes_z2, tes_z3 = tes_z1.to(device), tes_z2.to(device), tes_z3.to(device)
            tes_y_pre = net(tes_x, tes_z1, tes_z2, tes_z3)
